...
export class TransactionAnalysisService {
  ...
  private addressMap: Map<string, { tags: string[]; source: string }> =
    new Map();
  ...
  async analyzeMultiHopConnections(
    ...
    const firstHopConnections = primaryTransactions
      ? this.getFirstHopConnectionsFromTransactions(
          primaryAddress,
          primaryTransactions,
        )
    ...
    console.log(
      `Found ${firstHopConnections.length} first-hop addresses for ${primaryAddress}`,
    );
    ...
    for (const firstHop of firstHopConnections) {
      ...
      console.log(
        `Multi-hop: Checking first-hop address ${firstHop.address.toLowerCase()}, found tags: ${tags.join(", ") || "none"}`,
      );
      ...
      if (tags.length > 0 && this.isSanctionedTags(tags)) {
        ...
        console.log(
          `Found 1-hop sanctioned connection: ${firstHop.address} with tags: ${tags.join(", ")}`,
        );
      } else {
        console.log(
          `Multi-hop: Address ${firstHop.address} is not sanctioned (tags: ${tags.join(", ") || "none"})`,
        );
      }
      ...
      if (
        maxHops >= 2 &&
        !this.processedAddresses.has(firstHop.address.toLowerCase())
      ) {
        ...
        try {
          const secondHopConnections = await this.getFirstHopConnections(
            firstHop.address,
          );
          console.log(
            `Checking ${secondHopConnections.length} second-hop addresses from ${firstHop.address}`,
          );

          for (const secondHop of secondHopConnections) {
            const secondAddressInfo = this.addressMap.get(
              secondHop.address.toLowerCase(),
            );
            const secondTags = secondAddressInfo ? secondAddressInfo.tags : [];
            console.log(
              `Multi-hop: Checking second-hop address ${secondHop.address.toLowerCase()}, found tags: ${secondTags.join(", ") || "none"}`,
            );
            ...
            if (secondTags.length > 0 && this.isSanctionedTags(secondTags)) {
              ...
              console.log(
                `Found 2-hop sanctioned connection: ${primaryAddress} → ${firstHop.address} → ${secondHop.address} with tags: ${secondTags.join(", ")}`,
              );
            }
          }
        } catch (error) {
          console.warn(
            `Failed to analyze second-hop for ${firstHop.address}:`,
            error,
          );
        }
      }
    ...
    console.log(
      `Multi-hop analysis complete. Found ${connections.length} connections.`,
    );
    return connections;
  }

  private getFirstHopConnectionsFromTransactions(
    address: string,
    transactions: Array<{
      hash: string;
      from: string;
    ...
    console.log(
      `Multi-hop: Parsing ${transactions.length} pre-fetched transactions for ${address}`,
    );
    ...
    for (const tx of transactions) {
      ...
      console.log(
        `Multi-hop: Transaction ${tx.hash}: from=${fromAddr}, to=${toAddr}, current=${currentAddr}`,
      );
    ...
    console.log(
      `Multi-hop: Found ${connections.size} connected addresses from pre-fetched transactions: ${Array.from(connections).join(", ")}`,
    );
    return Array.from(connections).map((addr) => ({ address: addr }));
  }

  private async getFirstHopConnections(
    address: string,
    ...
    try {
      ...
      console.error(
        `Multi-hop: Failed to get transactions for ${address}:`,
        error,
      );
    }

    console.log(
      `Multi-hop: Found ${connections.size} connected addresses for ${address}`,
    );
    return Array.from(connections).map((addr) => ({ address: addr }));
  }
...
export const createTransactionAnalysisService = (
  addressData: Map<string, { tags: string[]; source: string }>,
) => {
